# Script to Generate Natural & Consistent Backdated Commits (March 25, 2026 to August 17, 2026)
param(
    [string]$StartDateStr = "2026-03-25",
    [string]$EndDateStr = "2026-08-17",
    [switch]$DryRun = $false
)
...
$commitMessages = @(
    ...
    "refactor(services): separate cloud storage adapter from UI layer",
    "refactor(export): optimize collage rendering algorithm for speed",
    "refactor(i18n): restructure localization strings dictionary",
    "refactor(events): use custom event bus for decoupled component messaging",
    "refactor(cleanup): remove deprecated legacy camera capture methods",
    ...
    "perf(api): cache static filter presets with aggressive HTTP headers",
    "perf(dom): virtualize gallery list items for smooth 60fps scrolling",
    "perf(state): optimize selector subscriptions to minimize re-renders",
    ...
    "style(controls): refine slider track and thumb sizing for touch screens",
    "style(badge): update status indicator badge colors and glow effects",
    "style(shutter): design circular pulsating shutter button with feedback",
    ...
    "docs(comments): add inline JSDoc comments to image processing helpers",
    "docs(testing): describe manual verification steps for photobooth kiosk",
    ...
    "test(api): add integration test coverage for photo seed endpoint",
    "chore(build): optimize Vite build chunks and asset minification settings"
)
...
Write-Host "Natural Backdated Commit Generator" -ForegroundColor Yellow
Write-Host "Range: $StartDateStr to $EndDateStr ($totalDays days)" -ForegroundColor Cyan
Write-Host "Mode: $(if ($DryRun) {'DRY-RUN (Simulasi)'} else {'LIVE EXECUTION'})" -ForegroundColor $(if ($DryRun) {'Magenta'} else {'Green'})
Write-Host "==========================================================" -ForegroundColor Cyan

$totalCommitsGenerated = 0
$msgCount = $commitMessages.Length

# Read current total commits from log if possible, or initialize
$counter = 2000
...
for ($d = 0; $d -lt $totalDays; $d++) {
    ...
    # Determine commit count based on day of week (Natural human variation)
    $isWeekend = ($dayOfWeek -eq [DayOfWeek]::Saturday -or $dayOfWeek -eq [DayOfWeek]::Sunday)
    
    if ($isWeekend) {
        # Weekend: 10 - 24 commits (or occasionally lighter)
        $dailyCommitCount = $random.Next(10, 25)
    } elseif ($dayOfWeek -eq [DayOfWeek]::Monday -or $dayOfWeek -eq [DayOfWeek]::Friday) {
        # Mon/Fri: 28 - 42 commits
        $dailyCommitCount = $random.Next(28, 43)
    } else {
        # Tue/Wed/Thu: 35 - 50 commits (Peak active days)
        $dailyCommitCount = $random.Next(35, 51)
    }

    # Time slots for natural human distribution:
    # Slot 1: Morning (09:00 - 12:30) ~ 30%
    # Slot 2: Afternoon (13:45 - 18:00) ~ 45%
    # Slot 3: Evening (19:30 - 22:45) ~ 25%
    ...
    if ($DryRun) {
        Write-Host "[$dateStr ($($dayOfWeek.ToString().PadRight(9)))] -> $dailyCommitCount commits planned (Morning: $slot1Count, Afternoon: $slot2Count, Evening: $slot3Count)" -ForegroundColor Gray
    }
    ...
    if (-not $DryRun -and ($d % 10 -eq 0 -or $d -eq ($totalDays - 1))) {
        $percent = [Math]::Round((($d + 1) / $totalDays) * 100, 1)
        Write-Host "Progress: [$($d + 1) / $totalDays days] ($percent%) - Total Commits: $totalCommitsGenerated..." -ForegroundColor Green
    }
}
...
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "Completed! Total $totalCommitsGenerated natural commits $(if ($DryRun) {'simulated'} else {'created'})." -ForegroundColor Yellow
Write-Host "==========================================================" -ForegroundColor Cyan
