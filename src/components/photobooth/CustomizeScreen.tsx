...
export default function CustomizeScreen() {
  ...
  useEffect(() => {
    const generatePreview = async () => {
      ...
      setPreviewUrl(canvas.toDataURL('image/jpeg', 0.8)); // lower quality for preview
        ...
        <div className="flex-1 w-full h-full flex items-center justify-center p-4 md:p-8 relative">
          {previewUrl ? (
            <motion.img 
              ...
              animate={{ opacity: isProcessing ? 0.6 : 1, scale: 1 }}
              src={previewUrl} 
              alt="Grid Preview" 
              className="max-w-full max-h-full object-contain"
              style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(200,121,65,0.2)' }}
            />
          ) : (
            <div className="flex items-center gap-2 text-[#c87941]">
              <Wand2 className="w-5 h-5 animate-pulse" />
              <span className="text-sm font-bold tracking-widest uppercase font-body">Generating Preview...</span>
            </div>
          )}
        </div>
      </div>
