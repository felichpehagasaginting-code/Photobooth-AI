...
export default function CapturedPreview() {
  ...
  if (!currentPhoto) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#030611' }}>
      <p className="text-[#7687a1] font-body">{t('Tidak ada foto', 'No photo captured')}</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: '#030611' }}>
      ...
      {/* Cool accent blob */}
      <div className="absolute pointer-events-none z-0" style={{ top: '-80px', left: '50%', transform: 'translateX(-50%)', width: '60%', height: 300, background: 'radial-gradient(circle, rgba(43,92,246,0.06) 0%, transparent 70%)' }} />
      ...
      <div className="relative z-10 sticky top-0" style={{ background: 'rgba(3,6,17,0.85)', backdropFilter: 'blur(14px)', borderBottom: '1px solid rgba(43,92,246,0.1)' }}>
        <div className="flex items-center gap-4 p-4 w-full max-w-7xl mx-auto">
          ...
          <div className="flex-1">
            <h2 className="font-display font-black text-[#f1f4fb] tracking-tight" style={{ fontSize: '1.05rem' }}>
              {t(`Foto ${currentTake} dari ${takeCount}`, `Photo ${currentTake} of ${takeCount}`)}
            </h2>
            <p className="text-[11px] text-[#7687a1] font-body">{t('Puaskah dengan hasilnya?', 'Happy with the result?')}</p>
          </div>

          {/* Take progress — line segments */}
        ...
        {!isLastTake ? (
          ...
          >
            ...
            <div style={{
              ...
              {/* Header bar */}
              <div className="flex items-center px-3 py-2" style={{ background: 'rgba(43,92,246,0.06)', borderBottom: '1px solid rgba(43,92,246,0.1)' }}>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full" style={{ background: 'rgba(43,92,246,0.4)' }} />
                  <div className="w-2 h-2 rounded-full" style={{ background: 'rgba(156,182,249,0.3)' }} />
                  <div className="w-2 h-2 rounded-full" style={{ background: 'rgba(45,212,191,0.3)' }} />
                </div>
                <div className="flex-1 text-center">
                  <span className="text-[9px] text-[#7687a1] tracking-[0.3em] uppercase font-body">
              ...
              {/* Bottom info */}
              <div className="flex items-center gap-2 px-3 py-2" style={{ background: 'rgba(43,92,246,0.04)', borderTop: '1px solid rgba(43,92,246,0.08)' }}>
                <div className="w-1.5 h-1.5 rounded-full pulse-dot" style={{ background: '#2dd4bf' }} />
                <span className="text-[11px] text-[#7687a1] font-body">
          ...
          <div className="w-full max-w-4xl mx-auto flex flex-col h-full justify-center">
            <h3 className="text-center font-display font-black text-xl mb-4 text-var(--copper)">
              {t('Review Foto Kamu', 'Review Your Photos')}
            </h3>
            <p className="text-center text-xs text-[#7687a1] font-body mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {capturedPhotos.map((photo, i) => (
                ...
                >
                  ...
                  <div className="absolute top-2 left-2 bg-[#030611]/85 px-2 py-0.5 text-[10px] font-bold text-var(--copper)">
                    {i + 1}
                  </div>
                  {selectedPhotoIndex === i && (
                    <div className="absolute inset-0 bg-rgba(43,92,246,0.2) flex items-center justify-center">
                      <div className="bg-[#030611] p-2 rounded-full border border-var(--copper)">
                        <RotateCcw className="w-6 h-6 text-var(--copper)" />
                      </div>
                    </div>
                  )}
                </motion.div>
      ...
      {/* ── Actions ── */}
      <div className="relative z-10 sticky bottom-0 p-4" style={{ background: 'linear-gradient(to top, #030611 60%, rgba(3,6,17,0.9) 80%, transparent)' }}>
